...
public class RecruiterController {
    ...
    public ResponseEntity<Map<String, String>> generateAndServePdf(@PathVariable Long id) {
        Recruiter recruiter = recruiterRepo.findById(id)
                .orElseThrow(() -> new RuntimeException("Recruiter not found"));

        try {
            Files.createDirectories(Paths.get(PDF_DIR)); // Example: "uploads/recruiter_pdfs/"
            ...
            URI fileUri = URI.create("/uploads/recruiter_pdfs/" + filename);
            return ResponseEntity.status(HttpStatus.FOUND).location(fileUri).build();
    ...
    // --- Serve profile image for recruiter ---
    @GetMapping("/{id}/profile-image")
    public ResponseEntity<UrlResource> serveProfileImage(@PathVariable Long id) throws IOException {
        Recruiter recruiter = recruiterRepo.findById(id)
                .orElseThrow(() -> new RuntimeException("Recruiter not found"));

        String imageUrl = recruiter.getProfileImagePath();
        if (imageUrl == null || imageUrl.isEmpty()) {
            return ResponseEntity.notFound().build();
        }

        // Extract local path by removing server URL prefix
        String localPathString = imageUrl.replace("http://localhost:8081/", "");
        Path path = Paths.get(localPathString);
